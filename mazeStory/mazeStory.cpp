	...
	if (SceneNow == scene5) {
		...
		for (int i = 0; i < 10; i++) { //체력 생성
			if (i < 3) {
				...
				showObject(HP11[i]); //하트 보이기
			}
			if (i < 5) {
				HE0[i] = CreateObject1("image/H.png", scene11, 700 + (52 * i), 572); //칠득 하트
			}
			else HE0[i] = CreateObject1("image/H.png", scene11, 700 + (52 * (5-i)), 492); //칠득 하트
...
void timerCallback(TimerID timer)
{
	...
	if (timer == timerp) {
		...
		if (HitPrince && ((SceneNow == scene5) || SceneNow == scene11)) { //보스전에서 공격모션 재생
...
int main() {
	...
	scene12 = createScene("엔딩장소", "image/scene9.png"); // 엔딩씬
